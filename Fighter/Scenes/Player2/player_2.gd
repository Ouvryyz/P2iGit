...
@export var ShootScene = preload("res://Fighter/Scenes/Shoot/YellowShoot.tscn") 
...
@onready var anim_player = $Player2
...
func _input(event: InputEvent) -> void:
	direction = Input.get_vector("move_left2", "move_right2", "move_up2", "move_down2")
	...
	if Input.is_action_just_pressed("shoot2"):
		shoot_projectile()
	if event.is_action_pressed("heal2"):
		if can_heal:
			cast_heal_spell()
		else:
...
func dash():
	...
	if GameState.current_fighter_scene:
		GameState.current_fighter_scene.start_dash_cooldown_player2(dash_cooldown)
